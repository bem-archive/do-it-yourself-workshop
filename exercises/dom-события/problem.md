...
Опишите поведение блока ```form```. Нужно предотвратить отправку формы после нажатия на кнопку. Для этого нужно использовать метод ```preventDefault()```. Вместо этого выведите в консоль сообщение `'query send'`.
